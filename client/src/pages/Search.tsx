...
export function Search() {
  ...
  const followUpMutation = useMutation({
    onMutate: async () => ({ hadSession: !!sessionId }),
    mutationFn: async (followUpQuery: string) => {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          if (!newResponse.ok) throw new Error(await readApiErrorMessage(newResponse));
    ...
    onSuccess: (result, _followUpQuery, context) => {
      setCurrentResults(result);
      setIsFollowUp(Boolean(context?.hadSession));
    },
  });
